...
TTS quality practices so `fatterqwen` does not depend on external example
...
def estimate_text_speech_units(text: str) -> float:
    ...
    Usage:
        The service uses this heuristic to decide when a request should be split
        into multiple synthesis chunks even though the upstream Qwen API does not
        expose a native duration estimator.
