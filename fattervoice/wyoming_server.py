...
_WYOMING_ADVERTISED_LANGUAGE_TAGS = tuple(
    language_tag
    for language_tag, resolved_language in _WYOMING_LANGUAGE_TAG_ALIASES.items()
...
def resolve_wyoming_language(language: Optional[str]) -> Optional[str]:
    """Translate Wyoming/Home Assistant language identifiers into OmniVoice values.

    Usage:
        Wyoming clients and Home Assistant commonly send BCP47-style language tags
        such as `en-US`, while OmniVoice accepts language IDs like `en` as well
        as language names. Call this helper before constructing a synthesis
        request so Home Assistant language selection remains compatible with the
        shared OmniVoice-backed synthesis service.

    Parameters:
        language: The optional Wyoming/HA language identifier supplied by a client.

    Returns:
        The canonical OmniVoice language ID when the input can be recognized, or
        `None` when the input is empty or should fall back to OmniVoice
        auto-detection.
    """
    if language is None:
        return None
    ...
    if language_key in _WYOMING_LANGUAGE_TAG_ALIASES:
        return _WYOMING_LANGUAGE_TAG_ALIASES[language_key]
    if "-" in language_key:
        primary_subtag = language_key.split("-", 1)[0]
        if primary_subtag:
...
def advertised_wyoming_languages() -> list[str]:
    ...
    Usage:
        ...
        pipeline. This helper centralizes the conservative language-tag subset we
        expose while the underlying OmniVoice backend remains broadly multilingual.

    Parameters:
        None.

    Returns:
        A stable list of Wyoming/Home Assistant language tags advertised for the
        shared voice-clone implementation.
    """
    return list(_WYOMING_ADVERTISED_LANGUAGE_TAGS)


class FatterVoiceWyomingHandler(AsyncEventHandler):
    ...
    def _build_synthesis_request(
        ...
        Usage:
            ...
            language tags such as `en-US` into OmniVoice-friendly language IDs
            such as `en`.
...
def build_wyoming_info(voice_registry: VoiceRegistry) -> Info:
    ...
    return Info(
        tts=[
            TtsProgram(
                name="fattervoice",
                description="Wyoming TTS server backed by OmniVoice voice cloning",
                attribution=Attribution(
                    name="fattervoice",
                    url="https://github.com/justinlime/fatterqwen",
                ),
